...
public class ServiceUserFollow {
    ...
    /** Get accepted follower IDs. Cached 20s. */
    ...
    public List<UserFollow> getPendingRequests(int userId) {
        ...
        return InMemoryCache.getOrLoad(key, 15, () -> {
            List<UserFollow> list = new ArrayList<>();
            if (useApi()) {
                try {
    ...
    /** Get the status of a follow row (follower→followed), or null if none. */
